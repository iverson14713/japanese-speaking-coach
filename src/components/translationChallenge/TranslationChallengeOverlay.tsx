...
interface TranslationChallengeOverlayProps {
  open: boolean
...
export function TranslationChallengeOverlay({
  open,
  initialLanguage,
  onClose,
  onPracticeRecorded,
}: TranslationChallengeOverlayProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setRecordSecondsLeft((current) => current - 1)
    }, 1000)

    return () => {
      window.clearTimeout(timer)
    }
  }, [phase, isListening, recordSecondsLeft, stopListening])
  ...
  useEffect(() => {
    ...
    finalizeRecording(transcript, speechError)
  }, [phase, isListening, transcript, speechError, finalizeRecording])
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setPrepCountdown((current) => current - 1)
    }, 1000)

    return () => {
      window.clearTimeout(timer)
    }
  }, [phase, prepCountdown, startListening])
  ...
  return (
    <div className="translation-challenge-overlay" role="presentation">
      <div className="translation-challenge-overlay__panel" role="dialog" aria-modal="true">
        ...
        <div className="translation-challenge-overlay__body">
          ...
          {phase === 'prep' && currentQuestion ? (
            <section className="translation-challenge-play" aria-live="polite">
              <p className="translation-challenge-play__meta">
                第 {questionIndex + 1} / {questions.length} 題
              </p>
              <p className="translation-challenge-play__prompt-label">請用{LANGUAGE_LABELS[language]}說出</p>
          ...
          {phase === 'recording' && currentQuestion ? (
            <section className="translation-challenge-play translation-challenge-play--recording" aria-live="polite">
              <p className="translation-challenge-play__meta">
                第 {questionIndex + 1} / {questions.length} 題 · 錄音中
              </p>
              <p className="translation-challenge-play__chinese">{currentQuestion.chinese}</p>
              <div className="translation-challenge-recording">
                <span className="pulse-dot" aria-hidden="true" />
                <span className="translation-challenge-recording__label">正在聽你說…</span>
                <span className="translation-challenge-recording__timer">{recordSecondsLeft}s</span>
              </div>
              {interimTranscript ? (
          ...
          {phase === 'result' && currentQuestion && currentResult ? (
            <section className="translation-challenge-result">
              ...
              <div className="translation-challenge-result__card">
                ...
                <p className="translation-challenge-result__label">你說的</p>
